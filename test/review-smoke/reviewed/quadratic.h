#ifndef SRC_QUADRATIC_H_
#define SRC_QUADRATIC_H_
...
// Both out-params must be non-null.
void PlotRange(const Solution& solution, double* x_min, double* x_max);

// Samples the curve over [x_min, x_max] into `samples` points. Returns an empty
// vector if `samples` < 2 or the range is empty.
std::vector<Point> Curve(const Solution& solution, double x_min, double x_max,
                         int samples);

// Returns a human-readable description of the root structure.
